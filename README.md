...
First, the binary display is arranged with least significant bit on the left, which (of course) is opposite to ordinary practice. This reversal was adopted as it made the display more 'natural' in use, given its placement immediately above the DIVIDE potentiometer, which generates an increasing value as it is rotated clockwise. 
