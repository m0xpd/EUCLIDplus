...
EUCLID+ divides an applied clock signal by any integer clock divisor (up to 30) making EUCLID+ useful as a voltage-controlled hardware clock divider. The start of the clock divide cycle can be reset manually, using the RESET pushbutton, or under CV control via the Reset input.
